...
def _compute_statistics_of_path(path, model, batch_size, dims, device):
    if path.endswith('.npz'):
        ...
        a, m, s = calculate_activation_statistics(files, model, batch_size,
                                               dims, device)
...
def calculate_fid_given_paths(paths, batch_size, device, dims):
    ...
    a1, m1, s1 = _compute_statistics_of_path(paths[0], model, batch_size,
                                         dims, device)
    a2, m2, s2 = _compute_statistics_of_path(paths[1], model, batch_size,
                                         dims, device)
...
def main():
    ...
    fid_value,prdc_value = calculate_fid_given_paths(args.path,
                                          args.batch_size,
                                          device,
                                          args.dims)
    print('FID: ', fid_value, 'PRDC: ', prdc_value)
